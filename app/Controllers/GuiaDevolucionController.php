...
use App\Models\GuiasDevolucion;
use Core\Clases\Imprimir;
use Valitron\Validator;
use App\Services\CarritoServicegdevolucion;
use DateTime;
...
class GuiaDevolucionController extends Controller
{
    ...
    function index()
    {
        ...
        return view('guiasd/index', ['titulo' => 'Guia x Devolucion', 'carritov' => $carritov, 'total' => $total, 'items' => $numero_items]);
    }
    ...
    function agregaritem(Request $request)
    {
        ...
        CarritoServicegdevolucion::agregar($producto);
        $total = number_format(CarritoServicegdevolucion::total(), 2, '.', '');
        $numero_items = str_pad(CarritoServicegdevolucion::numeroItems(), 2, '0', STR_PAD_LEFT);
    ...
    function quitaritem(Request $request)
    {
        ...
        CarritoServicegdevolucion::quitar($pos);
        $carritogd = session()->get('carritogd', []);
        $total = number_format(CarritoServicegdevolucion::total(), 2, '.', '');
        $numero_items = str_pad(CarritoServicegdevolucion::numeroItems(), 2, '0', STR_PAD_LEFT);
        $cvista = \retornavista('guiasd', 'detalle');
