...
<script>
    ...
    function modalEdit(id) {
        const ruta = '/admin/flete/edit/' + id;
        axios.get(ruta)
            .then(function(respuesta) {
                $('#modal-mantenimiento-contenido').html(respuesta.data)
                $('#modal-mantenimiento').modal('show');
    ...
    function store(modo, id) {
        ...
        if (cnombre.length == 0) {
            toastr.error('Ingrese un Nombre de Flete', 'Mensaje del sistema');
            return;
        }
        ...
        if (modo == 'N') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post('/admin/flete/store', data)
                        .then(function(respuesta) {
                            $('#modal-mantenimiento').modal('hide');
            ...
            const ruta = '/admin/flete/update/' + id;
            Swal.fire({
                title: '¿Actualizar Flete?',
                text: "Se ingresará con los nuevos datos",
                icon: 'question',
                showCancelButton: true,
